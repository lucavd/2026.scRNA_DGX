...
Luca Vedovelli^1,2^, Corrado Lanera^1,2^, Daniele Sabbatini^1,2,3^, and Dario Gregori^1,2^

^1^ Unit of Biostatistics, Epidemiology and Public Health, Department of Cardiac, Thoracic, Vascular Sciences and Public Health, University of Padova, Padova, Italy. `{luca.vedovelli,corrado.lanera,daniele.sabbatini,dario.gregori}@ubep.unipd.it`
...
In this hybrid pipeline, data loading, QC, normalisation, HVG selection and scaling executed on the host CPU. This reflects the structure of the available software rather than a design preference: at 500k and 1.3M cells the standard single-GPU pipeline exceeded the 80 GB VRAM of a single H100 (Table 4), so the Dask-CUDA path was the only way to run these dataset sizes at all, and within that path only PCA and neighbour graph construction had distributed GPU implementations usable for the pipeline as configured here. The constraint is therefore one of the current software stack, not a property of the operations themselves: on a single GPU, normalisation is 329x faster than CPU (Fig. 1), and distributing preprocessing across GPUs is precisely the direction we identify for future frameworks (see Discussion).
...
We conducted a binary search from 3.4M to 13.7M cells to identify the failure point. CPU memory was recorded as the resident set size of the parent Python process (`psutil.Process().memory_info().rss`), sampled immediately before and after each pipeline step and preceded by an explicit garbage collection; the reported peak is the maximum over these post-step samples. This instrumentation yields a lower bound on the job's true memory use for two reasons: it cannot observe allocations that are made and released within a single step, and it excludes the resident memory of the eight Dask-CUDA worker processes, which the SLURM cgroup does account for.
...
The failure point is counter-intuitive at first reading: the recorded peak RAM at 11.9M was 535 GB (26% of the 2 TB machine, 30% of the 1.8 TB SLURM job allocation), leaving apparent headroom, while recorded peak aggregate VRAM was only 49 GB out of 640 GB (7.6%). Neither figure explains why 12M cells fails, and our instrumentation cannot close the gap. As described in Methods, 535 GB is the largest between-step resident set size of the parent process alone. Two features of the 11.9M trace make the shortfall explicit. First, RAM rises from 124 GB at the end of data loading to 426 GB at the start of quality control, an increase of 302 GB that belongs to no timed step. Second, normalisation and HVG selection each register a between-step delta below 0.1 GB, which is possible only if their working memory is allocated and released inside the call. The step-level trace therefore records the pipeline's resting footprint between operations, not the peaks against which the SLURM cgroup enforces its limit. We report an indicative budget in Table 6 and treat the attribution of the 12M failure to a specific operation as unresolved. What the data do exclude is clustering as the CPU-side culprit: at 11.9M the three Leiden calls together added less than 1 GB of host RAM, because Leiden executes on GPU through cuGraph (aggregate VRAM rose from 28.8 GB to 48.8 GB across those calls). The 13.7M failure is a distinct event, occurring during the CPU-side sparse-to-dense scaling step.
...
: **Table 6.** Indicative memory budget at the 12M-cell failure point. The baseline components are a reconstruction that sums to the 535 GB recorded at the last successful run (11.9M cells). The transient contributions are estimated from operation-specific memory formulas; they are not measured, because the instrumentation samples only between steps and only the parent process (Methods). The budget does not account for the full working set at the moment of the kill, and is presented to indicate which operations dominate rather than to attribute the failure. {#tbl:memory_budget}

| Memory usage at 12M cells | GB |
|:--------------------------|---:|
| **Baseline (largest between-step sample, measured at 11.9M)** | |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*Measured subtotal (11.9M)* | **535** |
| **Contributions not represented in that subtotal** | |
| &nbsp;&nbsp;&nbsp;&nbsp;Unattributed rise between data loading and QC (measured at 11.9M) | +302 |
| &nbsp;&nbsp;&nbsp;&nbsp;`adata.raw = adata.copy()` (estimated) | +400 |
| &nbsp;&nbsp;&nbsp;&nbsp;Sparse to dense scaling, `sc.pp.scale` (estimated) | +200 |
| &nbsp;&nbsp;&nbsp;&nbsp;CSR to CSC layout conversion (estimated) | +190 |
| &nbsp;&nbsp;&nbsp;&nbsp;Scanpy `highly_variable_genes` internal temporaries | n.q. |
| &nbsp;&nbsp;&nbsp;&nbsp;Eight Dask-CUDA worker processes | n.q. |
| &nbsp;&nbsp;&nbsp;&nbsp;Leiden internal structures (executes on GPU; host delta at 11.9M < 1 GB) | ~50 |
| SLURM job allocation (`#SBATCH --mem=1800G`) | 1,800 |
| Physical machine memory | 2,048 |

Two contributions could not be quantified (n.q.) with the present instrumentation. Scanpy's `highly_variable_genes` was observed to trigger an out-of-memory event from a 629 GB baseline in a variant pipeline at 14M cells, implying internal temporaries in excess of 1,100 GB; it is the largest single transient we have direct evidence for. The resident memory of the eight Dask-CUDA worker processes is charged to the SLURM cgroup but is invisible to parent-process sampling. Either could dominate the budget above.
...
Our study contains several limitations that should be considered when generalising these findings. The scRNA-seq benchmark used a single biological dataset (mouse brain E18); performance profiles may differ substantially for datasets with different sparsity patterns, gene counts, or biological characteristics. Concordance analysis was performed at 10,000 cells; larger datasets may exhibit different adjusted Rand index distributions or reveal additional numerical differences. The stress test employing the memory-optimised pipeline is mathematically equivalent to but not implementation-identical to the standard rapids-singlecell workflow, introducing potential differences in how the optimization generalises to other analyses. Spatial benchmarks were conducted on a local RTX 4090 workstation rather than the DGX H100 cluster, precluding measurements of multi-GPU spatial scaling and benchmarking of the full Visium HD 2 um resolution (6.3 million bins). This was not a design choice: the cluster's driver (535.183.01) caps the CUDA runtime at 12.2, and while the scRNA-seq pipeline ran there through forward compatibility, the GPU components of the spatial stack did not, and the container could not be downgraded to a driver-compatible RAPIDS release without breaking the spatial dependencies (Methods). A conservative driver policy on shared production hardware is reasonable, since driver upgrades routinely break unrelated Python environments for every other user of the node, but it is itself a practical constraint on deploying GPU spatial pipelines, and one that the current software stack makes harder to work around than it should be. Scaling analysis was confined to a single DGX H100 node (1-8 GPUs); we did not benchmark multi-node configurations (e.g., 16 GPUs across two nodes interconnected by InfiniBand) because the near-flat wall time observed from 2 to 8 GPUs already indicates that the dominant bottleneck lies in CPU-side preprocessing and single-GPU graph operations (Leiden, UMAP), neither of which would benefit from additional nodes and both of which would be further penalised by inter-node communication latency. A further limitation concerns the multi-GPU configuration itself. Because the standard single-GPU pipeline exceeded 80 GB VRAM at 500k and 1.3M cells, those dataset sizes could only be run through the Dask-CUDA hybrid, in which the preprocessing steps had no distributed GPU route and therefore executed on the host CPU. The 75% CPU-preprocessing fraction we report is thus a property of the software stack available at the time of benchmarking rather than an intrinsic limit of GPU acceleration, and it should be expected to shrink as distributed preprocessing implementations mature. A second limitation concerns the memory instrumentation of the stress test itself: CPU memory was sampled between pipeline steps and for the parent process only, so the reported peaks are lower bounds and the attribution in Table 6 is indicative rather than measured. Resolving it would require continuous sampling of the whole process tree, or the peak usage recorded by the scheduler's cgroup, neither of which we collected. Finally, we did not benchmark integration methods (Harmony, scVI) or multi-sample spatial experimental designs, which represent important directions for ongoing work.
