---
title: "GPU-accelerated single-cell RNA-seq analysis on NVIDIA DGX H100: a systematic benchmark of speed, scalability, and biological concordance"
author:
  - name: Luca Vedovelli
...
abstract: |
  ...
  yet the standard analysis framework Scanpy remains CPU-bound.
  ...
  workflows at atlas scale and identify CPU-side preprocessing as the primary bottleneck
...
Here we present a comprehensive benchmark addressing four questions simultaneously.
...
All experiments were conducted on an NVIDIA DGX H100 node with 8$\times$H100 80 GB GPUs, dual Intel Xeon Platinum 8480C processors (112 cores), and 2 TB of DDR5 RAM, using a Singularity container to ensure full reproducibility.
...
All experiments used the 10x Genomics 1.3-million mouse brain cell dataset (E18) [@Zheng2017], obtained as a pre-processed AnnData h5ad file from the RAPIDS single-cell examples repository.
...
: **Table 4.** Pipeline steps, parameters, and implementations. All steps used identical parameters for the CPU and GPU pipelines except where noted. {#tbl:pipeline}
...
The key optimisations were: (i) scaling on CPU rather than GPU (leveraging 2 TB system RAM); (ii) a distributed covariance PCA that scatters the scaled matrix across all 8 GPUs, computes local covariance contributions ($\mathbf{X}_i^\top \mathbf{X}_i$, each 2,000$\times$2,000), sums and eigendecomposes on GPU 0, then projects locally---mathematically equivalent to standard PCA via the covariance method; (iii) lean GPU transfer that replaces the dense matrix with an empty sparse placeholder after PCA, since downstream steps (neighbours, clustering, UMAP) only require the PCA embedding; and (iv) a reduced RMM pool (2 GB per worker).
...
All benchmarks ran on a single node of the UPSCALE/CONVECS DGX H100 cluster at the University of Padova, comprising 8$\times$NVIDIA H100 80 GB SXM GPUs connected via NVLink 4.0, dual Intel Xeon Platinum 8480C CPUs (112 cores total), and 2 TB DDR5 RAM.
...
These concordance levels are consistent with the expected range of variation between biological replicates of the same pipeline [@Tian2019; @Duo2020] and support the conclusion that GPU and CPU pipelines are interchangeable for biological interpretation.
...
Data loading from disk and Leiden clustering on small graphs show no GPU advantage or even a CPU advantage, consistent with the roofline model: these operations are I/O-bound or have insufficient arithmetic intensity to saturate GPU compute units [@Williams2009; @Lindegger2023].
...
Floating-point non-associativity in GPU parallel reductions contributes negligibly to the observed differences [@Shanmugavelu2024; @Collange2015], as the dominant source of variation is the Leiden algorithm's non-deterministic vertex ordering.
...
**Limitations.**
Our benchmark used a single biological dataset (mouse brain E18); performance profiles may differ for datasets with different sparsity patterns or gene counts.
...
We did not benchmark integration methods (Harmony, scVI) or spatial analysis tools, which are the subject of ongoing work.
...
These results provide practical guidance for the computational bioinformatics community as single-cell datasets grow toward the billion-cell scale [@Regev2017].


# Tables

: **Table 1.** Summary of benchmark results across all pipeline configurations. Times are mean $\pm$ SD over five repeats. Single-GPU results for 500k and 1.3M cells are absent because the standard pipeline exceeded 80 GB VRAM on a single H100. {#tbl:summary}
...
: **Table 2.** Stress test results: maximum cell count on a single DGX H100 node (8$\times$H100, 2 TB RAM) using the memory-optimised pipeline. {#tbl:stress}
