...
# NOTE: NO --gres=gpu — CPU-only job

# Full CPU benchmark: all dataset sizes × 5 repeats
# Sizes run smallest→largest so partial results are saved if time runs out.
...
for SIZE in "${SIZES[@]}"; do
    ...
    echo ""
    echo "================================================================"
    echo "=== CPU benchmark: ${SIZE} cells, ${N_REPEATS} repeats ==="
    echo "=== Start: $(date) ==="
    echo "================================================================"
    ...
    if [ ${EXIT_CODE} -ne 0 ]; then
        echo "ERROR: CPU benchmark failed for ${SIZE} cells"
        FAILED=$((FAILED + 1))
    fi
done
...
ls -lh "${WORKDIR}/results/cpu_"*.json 2>/dev/null
